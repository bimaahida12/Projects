    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                IdentityResult result = await IdentityManager.Authentication.CreateAndSignInExternalUserAsync(AuthenticationManager, new ApplicationUser() { UserName = model.UserName, PhotoUrl = @"../../img/Avatars/default-avatar.jpg" });
                if (result.Success)
                {
                    return RedirectToLocal(returnUrl);
                }
